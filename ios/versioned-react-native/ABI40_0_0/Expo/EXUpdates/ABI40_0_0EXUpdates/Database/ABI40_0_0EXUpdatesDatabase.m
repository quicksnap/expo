...
#import <ABI40_0_0EXUpdates/ABI40_0_0EXUpdatesDatabase+Tests.h>
#import <ABI40_0_0EXUpdates/ABI40_0_0EXUpdatesDatabaseInitialization.h>
...
@interface ABI40_0_0EXUpdatesDatabase ()

@property (nonatomic, assign) sqlite3 *db;

@end

static NSString * const ABI40_0_0EXUpdatesDatabaseManifestFiltersKey = @"manifestFilters";
static NSString * const ABI40_0_0EXUpdatesDatabaseServerDefinedHeadersKey = @"serverDefinedHeaders";

@implementation ABI40_0_0EXUpdatesDatabase
...
- (BOOL)openDatabaseInDirectory:(NSURL *)directory withError:(NSError ** _Nullable)error
{
  ...
  sqlite3 *db;
  if (![ABI40_0_0EXUpdatesDatabaseInitialization initializeDatabaseWithLatestSchemaInDirectory:directory database:&db error:error]) {
    return NO;
  }
  ...
  for (ABI40_0_0EXUpdatesAsset *asset in assets) {
    ...
    if ([self _executeSql:assetInsertSql
                 withArgs:@[
                          ...
                          asset.filename,
                          asset.contentHash,
                          @(ABI40_0_0EXUpdatesDatabaseHashTypeSha1)
                          ]
                    error:error] == nil) {
...
{
  if (update.status != ABI40_0_0EXUpdatesUpdateStatusDevelopment) {
    update.status = ABI40_0_0EXUpdatesUpdateStatusReady;
  }
...
- (void)markMissingAssets:(NSArray<ABI40_0_0EXUpdatesAsset *> *)assets error:(NSError ** _Nullable)error
{
  ...
  for (ABI40_0_0EXUpdatesAsset *asset in assets) {
    if ([self _executeSql:updatesSql withArgs:@[@(ABI40_0_0EXUpdatesUpdateStatusPending), @(asset.assetId)] error:error] == nil) {
      sqlite3_exec(_db, "ROLLBACK;", NULL, NULL, NULL);
      return;
    }
  }
...
- (nullable NSArray<ABI40_0_0EXUpdatesUpdate *> *)allUpdatesWithConfig:(ABI40_0_0EXUpdatesConfig *)config error:(NSError ** _Nullable)error
{
  ...
  NSMutableArray<ABI40_0_0EXUpdatesUpdate *> *launchableUpdates = [NSMutableArray new];
  for (NSDictionary *row in rows) {
...
- (nullable NSArray<ABI40_0_0EXUpdatesUpdate *> *)allUpdatesWithStatus:(ABI40_0_0EXUpdatesUpdateStatus)status config:(ABI40_0_0EXUpdatesConfig *)config error:(NSError ** _Nullable)error
{
  ...
  NSMutableArray<ABI40_0_0EXUpdatesUpdate *> *updates = [NSMutableArray new];
  for (NSDictionary *row in rows) {
...
- (nullable NSArray<ABI40_0_0EXUpdatesUpdate *> *)launchableUpdatesWithConfig:(ABI40_0_0EXUpdatesConfig *)config error:(NSError ** _Nullable)error
{
  ...
  FROM updates\
  WHERE scope_key = ?1\
  AND status IN (%li, %li, %li);", (long)ABI40_0_0EXUpdatesUpdateStatusReady, (long)ABI40_0_0EXUpdatesUpdateStatusEmbedded, (long)ABI40_0_0EXUpdatesUpdateStatusDevelopment];
...
- (nullable NSArray<ABI40_0_0EXUpdatesAsset *> *)allAssetsWithError:(NSError ** _Nullable)error
{
  ...
  NSMutableArray<ABI40_0_0EXUpdatesAsset *> *assets = [NSMutableArray arrayWithCapacity:rows.count];
...
{
  return [self _jsonDataWithKey:ABI40_0_0EXUpdatesDatabaseServerDefinedHeadersKey scopeKey:scopeKey error:error];
}

- (nullable NSDictionary *)manifestFiltersWithScopeKey:(NSString *)scopeKey error:(NSError ** _Nullable)error
{
  return [self _jsonDataWithKey:ABI40_0_0EXUpdatesDatabaseManifestFiltersKey scopeKey:scopeKey error:error];
}
...
{
  [self _setJsonData:serverDefinedHeaders withKey:ABI40_0_0EXUpdatesDatabaseServerDefinedHeadersKey scopeKey:scopeKey isInTransaction:NO error:error];
}

- (void)setManifestFilters:(NSDictionary *)manifestFilters withScopeKey:(NSString *)scopeKey error:(NSError ** _Nullable)error
{
  [self _setJsonData:manifestFilters withKey:ABI40_0_0EXUpdatesDatabaseManifestFiltersKey scopeKey:scopeKey isInTransaction:NO error:error];
}
...
- (void)setMetadataWithManifest:(ABI40_0_0EXUpdatesUpdate *)updateManifest error:(NSError ** _Nullable)error
{
  ...
  if (updateManifest.serverDefinedHeaders) {
    if (![self _setJsonData:updateManifest.serverDefinedHeaders
                   withKey:ABI40_0_0EXUpdatesDatabaseServerDefinedHeadersKey
  ...
  if (updateManifest.manifestFilters) {
    if (![self _setJsonData:updateManifest.manifestFilters
                   withKey:ABI40_0_0EXUpdatesDatabaseManifestFiltersKey
                  scopeKey:updateManifest.scopeKey
           isInTransaction:YES
                     error:error]) {
      sqlite3_exec(_db, "ROLLBACK;", NULL, NULL, NULL);
...
- (nullable NSArray<NSDictionary *> *)_executeSql:(NSString *)sql withArgs:(nullable NSArray *)args error:(NSError ** _Nullable)error
{
  ...
  dispatch_assert_queue(_databaseQueue);
  return [ABI40_0_0EXUpdatesDatabaseUtils executeSql:sql withArgs:args onDatabase:_db error:error];
}

- (NSError *)_errorFromSqlite:(struct sqlite3 *)db
{
  return [ABI40_0_0EXUpdatesDatabaseUtils errorFromSqlite:db];
}

- (ABI40_0_0EXUpdatesUpdate *)_updateWithRow:(NSDictionary *)row config:(ABI40_0_0EXUpdatesConfig *)config
{
  NSError *error;
  id metadata = nil;
  ...
  ABI40_0_0EXUpdatesUpdate *update = [ABI40_0_0EXUpdatesUpdate updateWithId:row[@"id"]
                                                 scopeKey:row[@"scope_key"]
                                               commitTime:[ABI40_0_0EXUpdatesDatabaseUtils dateFromUnixTimeMilliseconds:(NSNumber *)row[@"commit_time"]]
                                           runtimeVersion:row[@"runtime_version"]
                                                 metadata:metadata
                                                   status:(ABI40_0_0EXUpdatesUpdateStatus)[(NSNumber *)row[@"status"] integerValue]
                                                     keep:[(NSNumber *)row[@"keep"] boolValue]
                                                   config:config
                                                 database:self];
  update.lastAccessed = [ABI40_0_0EXUpdatesDatabaseUtils dateFromUnixTimeMilliseconds:(NSNumber *)row[@"last_accessed"]];
  return update;
}

- (ABI40_0_0EXUpdatesAsset *)_assetWithRow:(NSDictionary *)row
{
  NSError *error;
  id metadata = nil;
  ...
  ABI40_0_0EXUpdatesAsset *asset = [[ABI40_0_0EXUpdatesAsset alloc] initWithKey:key type:row[@"type"]];
  asset.assetId = [(NSNumber *)row[@"id"] unsignedIntegerValue];
  asset.url = url;
  asset.downloadTime = [ABI40_0_0EXUpdatesDatabaseUtils dateFromUnixTimeMilliseconds:(NSNumber *)row[@"download_time"]];
