...
@interface ABI39_0_0EXUpdatesAppLauncherWithDatabase ()

@property (nullable, nonatomic, strong, readwrite) ABI39_0_0EXUpdatesUpdate *launchedUpdate;
...
@property (nonatomic, copy) ABI39_0_0EXUpdatesAppLauncherCompletionBlock completion;
...
static NSString * const ABI39_0_0EXUpdatesAppLauncherErrorDomain = @"AppLauncher";

@implementation ABI39_0_0EXUpdatesAppLauncherWithDatabase
                   ...
                   completionQueue:(dispatch_queue_t)completionQueue
{
  dispatch_async(database.databaseQueue, ^{
    NSError *error;
    NSArray<ABI39_0_0EXUpdatesUpdate *> *launchableUpdates = [database launchableUpdatesWithConfig:config error:&error];
    ...
    dispatch_async(completionQueue, ^{
      ...
      ABI39_0_0EXUpdatesUpdate *embeddedManifest = [ABI39_0_0EXUpdatesEmbeddedAppLoader embeddedManifestWithConfig:config database:database];
      NSMutableArray<ABI39_0_0EXUpdatesUpdate *>*filteredLaunchableUpdates = [NSMutableArray new];
      for (ABI39_0_0EXUpdatesUpdate *update in launchableUpdates) {
        if (update.status == ABI39_0_0EXUpdatesUpdateStatusEmbedded) {
...
{
  NSAssert(!_completion, @"ABI39_0_0EXUpdatesAppLauncher:launchUpdateWithSelectionPolicy:successBlock should not be called twice on the same instance");
  _completion = completion;

  if (!_launchedUpdate) {
    [[self class] launchableUpdateWithConfig:_config database:_database selectionPolicy:selectionPolicy completion:^(NSError * _Nullable error, ABI39_0_0EXUpdatesUpdate * _Nullable launchableUpdate) {
      if (error || !launchableUpdate) {
        if (self->_completion) {
          dispatch_async(self->_completionQueue, ^{
            ...
            self->_completion([NSError errorWithDomain:ABI39_0_0EXUpdatesAppLauncherErrorDomain code:1011 userInfo:userInfo], NO);
          });
        }
      } else {
...
{
  if (_launchedUpdate.status == ABI39_0_0EXUpdatesUpdateStatusEmbedded) {
    NSAssert(_assetFilesMap == nil, @"assetFilesMap should be null for embedded updates");
    _launchAssetUrl = [[NSBundle mainBundle] URLForResource:ABI39_0_0EXUpdatesBareEmbeddedBundleFilename withExtension:ABI39_0_0EXUpdatesBareEmbeddedBundleFileType];
    ...
    return;
  } else if (_launchedUpdate.status == ABI39_0_0EXUpdatesUpdateStatusDevelopment) {
    dispatch_async(self->_completionQueue, ^{
  ...
  if (_launchedUpdate) {
    ...
    for (ABI39_0_0EXUpdatesAsset *asset in _launchedUpdate.assets) {
...
- (void)_ensureAssetExists:(ABI39_0_0EXUpdatesAsset *)asset withLocalUrl:(NSURL *)assetLocalUrl completion:(void (^)(BOOL exists))completion
{
  [self _checkExistenceOfAsset:asset withLocalUrl:assetLocalUrl completion:^(BOOL exists) {
    ...
    [self _maybeCopyAssetFromMainBundle:asset withLocalUrl:assetLocalUrl completion:^(BOOL success, NSError * _Nullable error) {
      ...
      [self _downloadAsset:asset withLocalUrl:assetLocalUrl completion:^(NSError * _Nullable error, ABI39_0_0EXUpdatesAsset *asset, NSURL *assetLocalUrl) {
        if (error) {
          if (asset.isLaunchAsset) {
...
- (void)_checkExistenceOfAsset:(ABI39_0_0EXUpdatesAsset *)asset withLocalUrl:(NSURL *)assetLocalUrl completion:(void (^)(BOOL exists))completion
{
...
- (void)_maybeCopyAssetFromMainBundle:(ABI39_0_0EXUpdatesAsset *)asset
                         withLocalUrl:(NSURL *)assetLocalUrl
                           completion:(void (^)(BOOL success, NSError * _Nullable error))completion
{
  ABI39_0_0EXUpdatesUpdate *embeddedManifest = [ABI39_0_0EXUpdatesEmbeddedAppLoader embeddedManifestWithConfig:_config database:_database];
  if (embeddedManifest) {
    ABI39_0_0EXUpdatesAsset *matchingAsset;
    for (ABI39_0_0EXUpdatesAsset *embeddedAsset in embeddedManifest.assets) {
...
- (void)_downloadAsset:(ABI39_0_0EXUpdatesAsset *)asset
          withLocalUrl:(NSURL *)assetLocalUrl
            completion:(void (^)(NSError * _Nullable error, ABI39_0_0EXUpdatesAsset *asset, NSURL *assetLocalUrl))completion
{
  if (!asset.url) {
    completion([NSError errorWithDomain:ABI39_0_0EXUpdatesAppLauncherErrorDomain code:1007 userInfo:@{NSLocalizedDescriptionKey: @"Failed to download asset with no URL provided"}], asset, assetLocalUrl);
  }
  dispatch_async([ABI39_0_0EXUpdatesFileDownloader assetFilesQueue], ^{
...
- (ABI39_0_0EXUpdatesFileDownloader *)downloader
{
  if (!_downloader) {
    _downloader = [[ABI39_0_0EXUpdatesFileDownloader alloc] initWithUpdatesConfig:_config];
  }
  return _downloader;
}
